...
from infer_utils.display_frame import frame_display_multi_line
...
from utils.general import non_max_suppression  # , non_max_suppression1
...
##############################################
    ...
    def detect(self):
        ...
        if self.opt["half"]: model.half()  # to FP16
        model.eval()
        ###############################################
        '''
        ############ scripted model####################
        ...
        ############### Create Data Loader ###################
        try: 
            self.opt["loader"] = video_loader(self.opt["VideoFile"], (self.opt["work_height"] ,self.opt["work_width"]), self.opt["batch_size"], self.opt["half"])
        except Exception as e:
            print(e)
            sys.exit(0)
        ...
        ###############################################
        display.set_class_colors(len(self.opt["classes"]))
        ################################################################################################3
        ...
        box_counter = counter()
        box_counter.between_frame_counter
     

        while(self.opt["loader"].next_frame()):
            #counter.between_frame_counter += 1
            #between_frame_counter.add_count
            #print(between_frame_counter)
            if self.is_run == False: break
            ...
            for j in range(self.opt["batch_size"]):
                ...
                #boxes = boxes[::-1]
                #normboxes = boxes[::-1]
                with open(f"/home/user/Downloads/yolov5_inference/data__{j}.pkl", 'wb') as f:
                    pickle.dump(boxes, f)
                ...
                normboxes = display.get_norm_box(boxes, self.opt["classes"])
                #output_image = display.show_filtred_frame(boxes, self.opt["classes"])
                output_image = display.show_current_frame(normboxes, self.opt["classes"])
...
if __name__ == '__main__':
    ...
    #opt["VideoFile"] = "/media/user3070/data/Teplovoz_video/teplovoz_vid/new.mp4"
    opt["VideoFile"] = "/home/user/Downloads/yolov5_inference/video_teplovoz/20201104-123434_nuc6_cam18_motion.mp4"
